...
cd "$(dirname "$0")"
INSTALL_ROOT=$(pwd)
LAB_ID='filesystem-navigation'
LAB_TITLE='Filesystem Navigation'
...
for cmd in adduser awk base64 basename cat chmod chown cp cut date dirname find grep head id ln ls mkdir mv passwd printf pwd readlink rm sed sha256sum sleep sort su tail touch tr wc; do command_required "$cmd"; done

mkdir -p /home /srv/filesystem-navigation/cases
